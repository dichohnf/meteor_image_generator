...
class Net2NetTransformer(pl.LightningModule):
    def __init__(self,
        ...
        self.first_stage_model = self.init_first_stage_from_ckpt(first_stage_config)
        self.cond_stage_model = self.init_cond_stage_from_ckpt(cond_stage_config)
    ...
    @staticmethod
    def init_first_stage_from_ckpt(config):
    ...
    def init_cond_stage_from_ckpt(self, config):
        if config == "__is_first_stage__":
            ...
            return SOSProvider(self.sos_token)
        else:
            model = instantiate_from_config(config)
            model = model.eval()
            model.train = disabled_train
